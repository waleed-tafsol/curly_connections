import 'package:flutter/cupertino.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

import 'app_colors.dart';

abstract final class AppFonts {
  static const String fontFamily = 'HelveticaNeue';

  static const TextStyle _base = TextStyle(fontFamily: fontFamily, height: 0);

  static final black12w500 = _base.copyWith(
    fontSize: 12.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.black,
  );
  static final black16w400 = black12w400.copyWith(fontSize: 16.sp);
  static final black22w400 = _base.copyWith(
    fontSize: 22.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );

  static final black22w300 = black22w400.copyWith(fontWeight: FontWeight.w300);

  static final black30w700 = _base.copyWith(
    fontSize: 30.sp,
    fontWeight: FontWeight.w700,
    color: AppColors.black,
  );

  static final black20w600 = _base.copyWith(
    fontSize: 20.sp,
    fontWeight: FontWeight.w600,
    color: AppColors.black,
  );

  static final black32w400 = _base.copyWith(
    fontSize: 32.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );
  static final black57w400 = _base.copyWith(
    fontSize: 57.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );

  static final black24w300 = black22w400.copyWith(
    fontSize: 24.sp,
    fontWeight: FontWeight.w300,
  );
  static final black16w500 = _base.copyWith(
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.black,
  );
  static final black12w400 = _base.copyWith(
    fontSize: 12.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );
  static final black16w600 = _base.copyWith(
    fontSize: 16.sp,
    fontWeight: FontWeight.w600,
    color: AppColors.black,
  );
  static final black13w400 = _base.copyWith(
    fontSize: 13.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );
  static final black15w500 = black14w500.copyWith(fontSize: 15.sp);
  static final black10w500 = _base.copyWith(
    fontSize: 10.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.black,
  );
  static final black20w400 = _base.copyWith(
    fontSize: 20.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );
  static final black20w500 = _base.copyWith(
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.black,
  );

  static final black28w400 = _base.copyWith(
    fontSize: 28.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );

  static final black28w300 = black28w400.copyWith(fontWeight: FontWeight.w300);
  static final black14w400 = _base.copyWith(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );
  static final black18w400 = _base.copyWith(
    fontSize: 18.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );
  static final black18w500 = _base.copyWith(
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.black,
  );
  static final purple18w400 = _base.copyWith(
    fontSize: 18.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.borderPurple,
  );
  static final purple15w400 = _base.copyWith(
    fontSize: 15.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.borderPurple,
  );
  static final black14w500 = _base.copyWith(
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.black,
  );
  static final black14w600 = _base.copyWith(
    fontSize: 14.sp,
    fontWeight: FontWeight.w600,
    color: AppColors.black,
  );
  static final black10w400 = _base.copyWith(
    fontSize: 10.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.black,
  );
  static final black36w500 = _base.copyWith(
    fontSize: 36.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.black,
  );
  static final white14w500 = _base.copyWith(
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.white,
  );
  static final white15w500 = _base.copyWith(
    fontSize: 15.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.white,
  );

  static final white20w500 = _base.copyWith(
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.white,
  );
  static final white16w500 = _base.copyWith(
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.white,
  );

  static final grey16w400 = _base.copyWith(
    fontSize: 16.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textGrey,
  );
  static final grey16w500 = _base.copyWith(
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.textGrey,
  );
  static final grey14w500 = _base.copyWith(
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.textGrey,
  );
  static final grey13w400 = _base.copyWith(
    fontSize: 13.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textGrey,
  );
  static final grey19w500 = _base.copyWith(
    fontSize: 19.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.textGrey,
  );
  static final grey20w400 = _base.copyWith(
    fontSize: 20.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textGrey,
  );
   static final grey20w500 = _base.copyWith(
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.textGrey,
  );
  static final grey14w400 = _base.copyWith(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textGrey,
  );
  static final grey15w400 = _base.copyWith(
    fontSize: 15.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textGrey,
  );

  static final grey15w500 = grey15w400.copyWith(fontWeight: FontWeight.w500);

  static final grey12w400 = _base.copyWith(
    fontSize: 12.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textGrey,
  );

  static final yellow14w400 = _base.copyWith(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textYellow,
  );
  static final grey10w400 = _base.copyWith(
    fontSize: 10.sp,
    fontWeight: FontWeight.w400,
    color: AppColors.textGrey,
  );
  static final grey10w500 = _base.copyWith(
    fontSize: 10.sp,
    fontWeight: FontWeight.w500,
    color: AppColors.textGrey,
  );
}
